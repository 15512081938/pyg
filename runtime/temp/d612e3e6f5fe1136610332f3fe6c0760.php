<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:79:"D:\phpStudy\PHPTutorial\WWW\pyg\public/../application/home\view\cart\index.html";i:1569508650;s:72:"D:\phpStudy\PHPTutorial\WWW\pyg\application\home\view\layout\layout.html";i:1570629132;}*/ ?>
<!DOCTYPE html>
<html>
                    ...
                    <div class="cart-list">
                        <ul class="goods-list yui3-g" cart_id="<?php echo $v['id']; ?>" number="<?php echo $v['number']; ?>">
                            <li class="yui3-u-1-24">
                                <input type="checkbox" <?php if(($v['is_selected'])): ?>checked="checked" <?php endif; ?> class="row_check"
                                name="" id="" value="" />
                            </li>
                            <li class="yui3-u-6-24">
                                <div class="good-item">
                                    <div class="item-img"><img src="<?php echo $v['goods']['goods_logo']; ?>" style="height: 80px;"/></div>
...
<script type="text/javascript">
    $(function () {
        ...
        var change_total = function () {
            ...
            var total_number = 0;
            var total_price = 0;
            $('.row_check:checked').each(function (i, element) {
                //v是一个checkbox标签
                //累加每一行的数量
                total_number += parseInt($(element).closest('ul').find('.current_number').val());
                //累加每一行的小计金额
                total_price += parseFloat($(element).closest('ul').find('.sum').html());
            });
            //将计算好的数量和金额，显示到页面
        ...
        function check_all() {
            ...
            var checked = $('.row_check:checked').length;  //选中checked  的:  标签.row_check
        ...
        //控制全选的选中状态
        check_all();
//      控制单行的选中状态  不能乱写，如果全选未选中，则刷新的时候，只要有一个值未选中，就会导致全选不中
//      row_check();

        //修改购买数量 ,需要传过去数量，和标签this
        var change_num = function (number, element) {
            ...
            $.ajax({
                ...
                "success": function (res) {
                    ...
                    //计算当前行的小计金额
                    var sum = number * parseFloat($(element).closest('ul').find('.price').html());
...
//            cart_id 1/0
        ...
        $('.current_number').change(function () {
            ...
            //检测数量格式
            var regex_num = /^\d+$/;
            if (!regex_num.test(number) || number == 0) {
                //数量格式不对，恢复原状
        ...
        $('.delete').click(function () {
            ...
            //发送ajax
            $.ajax({
                "url": "<?php echo url('home/cart/deletecart'); ?>",
                "type": "post",
                "data": data,
                "dataType": "json",
                "success": function (res) {
